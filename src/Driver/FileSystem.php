...
class FileSystem extends AbstractDriver
{
     ...
     * cached.
     */
    protected array $memStore = [];

    /**
     * The limit of keys to store in memory.
     */
    protected int $memStoreLimit = 0;
     ...
     */
    protected int $directorySplit = 0;
     ...
     * @param string[] $key
     */
    protected function makeKeyString(array $key): string
    {
        $keyString = '';
     ...
     */
    protected function makePath(?array $key = null): string
    {
        ...
        if (isset($this->memStore['keys'][$memkey])) {
            ...
            // In most cases the key will be used almost immediately or not at all,
            // so it doesn't need to grow too large.
            if (count($this->memStore['keys']) > $this->memStoreLimit) {
                $numOfItemsToPick = (int) (ceil($this->memStoreLimit / 2) + 1);
                foreach (array_rand($this->memStore['keys'], $numOfItemsToPick) as $empty) {
